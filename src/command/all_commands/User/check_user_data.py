...
class CheckUserData(CommandInterface):
    def __init__(self, database, logged_in_user: User):
        super().__init__(database, logged_in_user)

    def _asker(self):
        user_to_see = input(
            "What is the username of the user you want to see the data about? "
        )
        return user_to_see

    def _check_if_allowed(self):
        # print("Check if allowed called!")
        correct = RightsComparison(
            self.logged_in_user, "check user data"
        ).check_if_allowed()
        return correct

    def _check_user_personal_data(self, _user_id):
        user_entity = user_meta_data_repo.find_entity_by_id(
            user_meta_data_repo(self.database), _user_id
        )
        _birthday = user_entity.birthday.strftime("%Y-%m-%d")
        data = {
            "First name": user_entity.first_name,
            "Last name": user_entity.last_name,
            "birthday": _birthday,
    ...
    def _check_user_flag_data(self, _user_id):
        user_entity = user_flag_data_repo.find_entity_by_id(
            user_flag_data_repo(self.database), _user_id
        )

        isKicked = self._check_bool_value(user_entity.isKicked)
        ...
        isBanned = self._check_bool_value(user_entity.isBanned)
        ...
        data = {
            "isKicked": isKicked,
            "kick_date": kick_date,
            "remove_kick_date": remove_kick_date,
    ...
    def _print_data(self):
        ...
        print(
            f"{_username} is {self._calculate_age(UserRepository(self.database).get_entity_by_username(_username))} "
            f"years old!"
        )
    ...
    def _calculate_age(self, user: User):
        ...
        user_bday = (
            user_meta_data_repo(self.database).find_entity_by_id(user_id).birthday
        )
