...
class ChangeUserType(CommandInterface):
    ...
    def ask_type():
        _type = input(
            "What is the type you want to change the user to? a) Owner, b) Admin, c) Moderator, d) User "
        )
        if _type == "a":
            return "Owner"
        if _type == "b":
    ...
    def _check_if_allowed(self):
        correct = RightsComparison(
            self.logged_in_user, "change user type"
        ).check_if_allowed()
        return correct
    ...
    def check_if_allowed_to_change(self, user_to_change, user_type_to_change_to):
        ...
        ).check_changeable_by_user_type(user_to_change, user_type_to_change_to)
        # print(allowed)
        return allowed
    ...
    def run(self):
        ...
        if self._check_if_allowed():
            if self.check_if_allowed_to_change(user_to_change, type_to_change_to):
                ...
                print(
                    "User %s his/her type successfully changed to %s"
                    % (user_to_change.username, type_to_change_to)
                )
                return
            if not self.check_if_allowed_to_change(user_to_change, type_to_change_to):
                print(
                    "You don't have permission to do this! Or user is already the type."
                )
                wait(1)
                return
